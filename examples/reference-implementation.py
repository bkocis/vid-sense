"""
Reference Implementation: Home Surveillance with Ollama
Source: /home/snow/Documents/Projects/ollama-home-surveillance

This is a working example of a video understanding system using LLaVA via Ollama.
It processes video frames from a webcam and generates captions using a multimodal LLM.

Key Features:
- Frame-by-frame processing every N seconds
- Scene description and change detection
- Multiple query types for detailed analysis
- Integration with Ollama for local LLM inference
"""

import cv2
import ollama
import datetime


def query_the_image(query: str, image_list: list[str]) -> ollama.chat:
    """
    Query the LLaVA model with an image and text query.
    
    Args:
        query: Text query/question about the image
        image_list: List of image bytes (JPEG encoded frames)
    
    Returns:
        Response content from the LLM
    """
    try:
        res = ollama.chat(
            model='llava:7b-v1.6-mistral-q2_K',
            options={
                'temperature': 0,
                "top_k": 1,
                'top_p': 0.1,
                'mirostat_tau': 1.0,
                'num_ctx': 1024,
                'seed': 42,
                'num_predict': 128
            },
            messages=[
                {
                    'role': 'system',
                    'content': "You are a home surveillance system. Answer with very short sentences."
                },
                {
                    'role': 'user',
                    'content': query,
                    'images': image_list,
                }
            ]
        )
    except Exception as e:
        print(f"Error: {e}")
        return None
    return res['message']['content']


def observe_scene_change(initial_scene: str, image) -> None:
    """
    Perform detailed scene analysis with multiple queries.
    
    Args:
        initial_scene: Initial scene description
        image: Image bytes to analyze
    """
    queries = [
        "Do you see people in the image?",
        "Do you see someone stealing something?",
        "Do you see people moving?",
        "Is there any dog in the frame?"
        ]
    for query in queries:
        print_out_the_response(query, image_list=image)


def print_out_the_response(query_message: str, image_list: list[str]):
    """
    Query the model and print formatted response.
    
    Args:
        query_message: Question to ask about the image
        image_list: List of image bytes
    """
    response_llava = query_the_image(query_message, image_list)
    timestamp = str(datetime.datetime.now())
    response = {
        "timestamp": timestamp,
        "question": query_message,
        "answer": response_llava
    }
    print(response)
    return response


def image_processing_function(frame) -> None:
    """
    Process a single frame: encode and pass to LLM.
    
    Args:
        frame: OpenCV frame (numpy array)
    """
    # frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    image = [cv2.imencode('.jpg', frame)[1].tobytes()]
    pass_image_to_llava(image)


def pass_image_to_llava(image) -> None:
    """
    Generate scene description and perform detailed analysis.
    
    Args:
        image: List of image bytes
    """
    scene_description = query_the_image("What is on the image?", image_list=image)
    observe_scene_change(initial_scene=scene_description, image=image)


def frame_generator(cap: cv2.VideoCapture, debug_show_image: bool):
    """
    Generator that reads frames from video capture.
    
    Args:
        cap: OpenCV VideoCapture object
        debug_show_image: Whether to display frames in a window
    
    Yields:
        Tuple of (frame_number, frame)
    """
    frame_number = 0
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        frame_number += 1
        if debug_show_image:
            cv2.imshow('Webcam Live', frame)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
        yield frame_number, frame


def display_frame_generator(
        cap: cv2.VideoCapture, image_processing_function: callable, every_nth_second: int, debug_show_image: bool
) -> None:
    """
    Process frames at specified intervals.
    
    Args:
        cap: OpenCV VideoCapture object
        image_processing_function: Function to process each frame
        every_nth_second: Process every Nth second of video
        debug_show_image: Whether to show debug window
    """
    frame_rate = 30
    for frame_number, frame in frame_generator(cap, debug_show_image):
        if frame_number % (every_nth_second * frame_rate) == 0:
            image_processing_function(frame)


if __name__ == "__main__":
    # Initialize webcam
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        raise IOError("Cannot open webcam")
    
    # Process every 5 seconds
    evaluate_a_frame_in_n_seconds = 5

    display_frame_generator(
        cap,
        image_processing_function=image_processing_function,
        every_nth_second=evaluate_a_frame_in_n_seconds,
        debug_show_image=False
    )

